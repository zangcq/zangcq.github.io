...
categories: ["Perl", "程序人生"]
tags: ["perl"]
permalink: "/2017/08/10/perl-syntax-1/"
legacy: true
toc: true
classes: wide
---
...
显然，返回的array长度是 _满足条件的元素个数_ ，这就意味着你可以用`grep`检查array中是否包含某个元素：
```
...
`grep`和`map`的组合形成了 _list comprehensions_ 这种许多其他语言中欠缺的非常强大特性。（译者注：list comprehensions大致的意思是利用map和filter从现有的列表构造新的列表，表达的含义是对一个列表中满足某个条件的所有元素上应用某个操作，而形成一个新的列表。）
...
请注意，我们从来没有显式提供`$a`和`$b`给子程序和语句块。就像`$_`一样，`$a`和`$b`实际上是当一对值需要比较时被 _填入_ 的全局变量。
