...
categories: ["GPGPU"]
permalink: "/2020/08/13/nvcc-%e5%88%86%e6%ad%a5%e7%bc%96%e8%af%91/"
legacy: true
toc: true
classes: wide
---
...
![这里写图片描述]({{ "/assets/images/legacy/nvcc-flow.png" | relative_url }}) 完整的说明了如何从一个 .cu or .cpp 生成一个可执行的文件。 1.只打印，不运行 `nvcc -O2 -c backprop_cuda.cu -keep -arch sm_30 --dryrun` 2.打印出 具体流程
```
    1. 读取环境变量
