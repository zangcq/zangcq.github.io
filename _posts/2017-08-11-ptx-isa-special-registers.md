...
categories: ["GPU PTX ISA Analysis"]
tags: ["GPU", "PTX ISA"]
permalink: "/2017/08/11/ptx-isa-special-registers/"
legacy: true
toc: true
classes: wide
---
...
For sm _1x targets,`%gridid` is limited to the range [0..216-1]. For sm_20, `%gridid` is limited to the range [0..232-1]. sm_30 supports the entire 64-bit range.
...
`%clock, %clock_hi` `%clock` A predefined, read-only 32-bit unsigned cycle counter. %clock_hi The upper 32-bits of %clock64 special register.
...
Special registers %pm0 _64..%pm7_ 64 are unsigned 64-bit read-only performance monitor counters. Their behavior is currently undefined.

### Notes

The lower 32bits of %pm0 _64..%pm7_ 64 are identical to %pm0..%pm7.

### PTX ISA Notes

%pm0 _64..%pm7_ 64 introduced in PTX ISA version 4.0.

### Target ISA Notes

%pm0 _64..%pm7_ 64 require sm_50 or higher.
...
%globaltimer, %globaltimer_lo, %globaltimer_hi %globaltimer A predefined, 64-bit global nanosecond timer. %globaltimer_lo The lower 32-bits of %globaltimer. %globaltimer_hi The upper 32-bits of %globaltimer.
